...
1. 游戏版本是 Minecraft 1.21.1。
2. 加载器是 NeoForge 21.1.217。
...
文本 Component 和物品数据组件允许在结构尚未闭合时继续提示，但最终仍由 Minecraft 1.21.1 原生 Codec 校验。模组自定义参数若不公开内部结构，只能显示命令树能够提供的候选和类型回退说明。
...
`//` 入口只在服务端也安装相同版本并成功协商载荷后出现。还需要玩家能够使用游戏管理员命令方块，并位于目标方块 64 格范围内。
