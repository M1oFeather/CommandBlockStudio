...
文本 Component 适用于 `tellraw`、`title` 等命令。编辑器允许在 JSON 尚未闭合时继续补全字段和值，包括内容源、文本样式、记分板、选择器、NBT、点击事件与悬停事件。候选接受后仍由当前 Minecraft 版本的原生 `ComponentSerialization` Codec 做最终校验。
